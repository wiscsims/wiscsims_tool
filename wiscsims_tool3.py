...
from .wiscsims_tool3_dockwidget import WiscSIMSTool3DockWidget
...
class WiscSIMSTool3:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'WiscSIMSTool3_{}.qm'.format(locale))
        ...
        self.toolbar = self.iface.addToolBar(u'WiscSIMS Tool 3')
        self.toolbar.setObjectName(u'WiscSIMS Tool 3')

        # print "** INITIALIZING WiscSIMSTool3"
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('WiscSIMS Tool 3', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/wiscsims_tool3/img/icon.png'
        self.wiscsims_tool_action = self.add_action(
            icon_path,
            text=self.tr(u'WiscSIMS Tool 3'),
            callback=self.run,
    ...
    def init_map_tool(self):
        ...
        if self.wiscsims_tool_action.isChecked():
            print('user activate the plugin')
            ...
            print('user deactivate the plugin')
            ...
            try:
                ...
                print('prevtool:{}'.format(self.prev_tool))
    ...
    def change_tableview_selection(self, index):
        ...
        # self.dockwidget.Txt_P1_Stage_X.setText(str(stage1[0]))
        # self.dockwidget.Txt_P1_Stage_Y.setText(str(stage1[1]))
        # self.dockwidget.Txt_P2_Stage_X.setText(str(stage2[0]))
        # self.dockwidget.Txt_P2_Stage_Y.setText(str(stage2[1]))
        # self.dockwidget.Txt_P1_Canvas_X.setText(str(canvas1[0]))
        # self.dockwidget.Txt_P1_Canvas_Y.setText(str(canvas1[1]))
        # self.dockwidget.Txt_P2_Canvas_X.setText(str(canvas2[0]))
        # self.dockwidget.Txt_P2_Canvas_Y.setText(str(canvas2[1]))
    ...
    def create_new_layer(self, fpath=None):
        ...
        print('error', error)
        print('QgsVectorFileWriter.NoError', QgsVectorFileWriter.NoError)
    ...
    def get_true_shapefile_headers(self, headers):
        ...
        if error != QgsVectorFileWriter.NoError:
            print('error on VectorFileWriter')
            return
    ...
    def create_preset_layer(self):
        ...
        if error != QgsVectorFileWriter.NoError:
            print('QgsVectorFileWriter Error')
            return
    ...
    def get_current_tool(self):
        ...
        # print(self.dockwidget.Tab_Tool.currentTabName())
    ...
    def add_preset_points(self):
        ...
        if self.dockwidget.Cmb_Preset_Layer.currentIndex() == -1:
            print('Select a layer for Preest')
            return
    ...
    def add_preset_point(self, pt):
        ...
        print('after add point: {}'.format(len(self.preset_points)))

    def undo_add_preset_point(self):
        print('undo preset point')
        print('len: {}'.format(len(self.undo_preset)))
        if len(self.undo_preset) == 0:
            return
        # prev_comment = self.preset_points[0][0]
        # print('prev comment: {}'.format(prev_comment))
        current_layer_index = self.dockwidget.Cmb_Preset_Layer.currentIndex()
        if current_layer_index == -1:
            print('no layers are selected')
            return
    ...
    def handle_comment_change_preview(self):
        ...
        print(self.rb, self.rb.numberOfVertices(), self.rb.size())
    ...
    def mapToolChanged(self, tool, prev_tool):
        print(" >>>>>> MAP TOOL CHANGED <<<<<<<<<<")

        # print('map tool: ', str(self.canvas.mapTool()), re.search(
        #     r'wiscsims_tool', str(tool)))
        if re.search(r'wiscsims_tool', str(tool)) or re.search(r'WiscSIMSTool', str(tool)):
            self.dockwidget.setEnabled(True)
            print('Current tool is WiscSIMS Tool')
            return

        try:
            print('disconnect mapToolChanged')
            self.canvas.mapToolSet.disconnect(self.mapToolChanged)
            print('unset map tool')
            self.canvas.unsetMapTool(self.canvasMapTool)
        except Exception:
            print('WT: error on disconnect canvas')
            pass

        print('remove rubber bands')
        self.clear_preview_points()

        print('deactivate plugin button')
        self.wiscsims_tool_action.setChecked(False)
        self.dockwidget.setEnabled(False)
    ...
    def canvasClicked(self, pt):
        ...
        if mode == 'point':
            print('current mode: Point')
            self.add_preset_point(pt)
        elif mode == 'line':
            print('current mode: Line')
            self.preset_line(pt)
        elif mode == 'grid':
            print('current mode: Grid')
            self.preset_grid(pt)
    ...
    def canvasDoubleClicked(self, pt):
        print('canvas double clicked, {}'.format(pt))
        if self.get_current_tool() != 'preset':
            return
        self.clear_preview_points()
